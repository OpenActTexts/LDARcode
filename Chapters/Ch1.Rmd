...
* The assignment operator `<-` is analogous to an equal sign in mathematics. The command `Insample <- read.csv("Data/PropertyFundInsample.csv", header=T, na.strings=c("."), stringsAsFactors=FALSE)` means we give the name `Insample` to the data read. 
...
Rowlabel <- rbind("Claim Frequency","Claim Severity","Deductible","Coverage (000's)")
Table2aa <- cbind(Rowlabel,as.matrix(Table2a))
pander(Table2aa)
```
...
Rowlabel <- rbind("Village","City","County","Misc","School",
                  "Town","Fire5--No","Fire5--Yes","NoClaimCredit--No",
                  "NoClaimCredit--Yes","Total")
Table4 <- cbind(Rowlabel,as.matrix(Tableaa))
