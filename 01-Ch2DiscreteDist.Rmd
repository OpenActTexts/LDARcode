...
- <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="The assignment operator  is analogous to an equal sign in mathematics.", style="color:green;">`<-`</a>
...
* <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Function used to get or assign names of an object. In this illustration, it was used to get the variables names.", style="color:green;">`names()`</a>
...
If we had to guess, from this plot we might say that the maximum value of the log likelihood was around 0.7. From calculus, we know that the maximum likelihood estimator <a href="#" data-toggle="popover" data-placement="top" title="Values of the parameters that are 'most likely' to have been produced by the data", style="color:green;">(*mle*)</a> of the Poisson distribution parameter equals the average claim count. For our data, this is
...
* To get the estimated $\lambda$ we take the $\exp$(coefficient)  `lambda_hat<-exp(CountPoisson1$coefficients)`
...
For goodness of fit, consider Pearson’s chi-square statistic below. The degrees of freedom (*df*) equals the number of cells minus
...
*This file contains illustrative **R** code for computing important count distributions. When reviewing this code, you should open an **R** session, copy-and-paste the code, and see it perform. Then, you will be able to change parameters, look up commands, and so forth, as you go. This code uses the dataset `SingaporeAuto.csv`*

## Singapore Data {.tabset}
This section loads the `SingaporeAuto.csv` dataset and check the name of variables and the dimension of the data. To have a glimpse at the data, the first 8 observations are listed. 
...
names(Singapore)
dim(Singapore)  # check number of observations and variables in the data
Singapore[1:8,]  # list the first 8 observations
attach(Singapore)  # attach dataset
```
...
* <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Function used to get or assign names of an object. In this illustration, it was used to get the variables names.", style="color:green;">`names()`</a>
 <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Function used to retrieve or set the dimension of an object.", style="color:green;">`dim()`</a>
 <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="When you attach a dataset using the `attach()` function, variable names in the database can be accessed by simply giving their names.", style="color:green;">`attach()`</a>

## Goodness of fit for Poisson distribution {.tabset}
This sections shows how to check the adequacy of the Poisson model for the Singapore data by computing the Pearson goodness-of-fit statistic.
...
* <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Function used to fit Generalized linear models.", style="color:green;">`glm()`</a>
 <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="See this for other modeling options.", style="color:green;">`help(glm)`</a>
 <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Function used in order to get the results.", style="color:green;">`summary()`</a>
 <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="In the output, this reminds us what model we ran and what options were specified.", style="color:green;">`call`</a>
 <a href="#" title="" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="Shows the distribution of the deviance residuals for individual cases used in the model.", style="color:green;">`Deviance Residuals`</a>
* The next part of the output shows the coefficient (maximum likelihood estimate of $\log(\lambda)$), its standard error, the z-statistic and the associated p-value. 
* To get the estimated $\lambda$ we take the $\exp$(coefficient)  `lambda<-exp(CountPoisson1$coefficients)`

### Pearson goodness-of-fit statistic
The table below gives the distribution of fitted claims frequency using Poisson distribution $(np_k)$
...
table1p = cbind(n*(dpois(0,lambda)),
                ...
                n*(dpois(2,lambda)),
                n*(dpois(3,lambda)),
                n*(1-ppois(3,lambda))) # or n*(1-dpois(0,lambda)-dpois(1,lambda)-dpois(2,lambda)-dpois(3,lambda)))
table2p<-rbind(c(0,1,2,3,"4+"),round(table1p, digits = 3))
table2p
```
